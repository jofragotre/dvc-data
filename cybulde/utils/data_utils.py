...
def initialize_dvc_storage(dvc_remote_name: str, dvc_remote_url: str) -> None:
    if not run_shell_command("dvc remote list"):
        ...
        run_shell_command(
            f"dvc remote modify {dvc_remote_name} --local gdrive_user_credentials_file ./docker/credentials/gdrive_credentials.json"
        )
...
def make_new_data_version(dvc_raw_data_folder: str, dvc_remote_name: str) -> None:
    DATA_UTILS_LOGGER.info("Make new data version")
    try:
        DATA_UTILS_LOGGER.info(f"dvc status {dvc_raw_data_folder}.dvc")
